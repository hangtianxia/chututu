...
export const hasNewVersion = async (): Promise<INewVersionRes> => {
  ...
  const res = await tryAsyncCatch(axios.get<IReleaseData>('https://api.github.com/repos/hangtianxia/chututu/releases/latest'));
  ...
  const assetsName = `chututu-${updateInfo.version}-${process.platform}-x64.${ext}`;
