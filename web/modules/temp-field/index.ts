...
function resolveFontSizePx(bgHeight: number, size: unknown) {
  const n = Number(size);
  if (!Number.isFinite(n) || n <= 0) return 0;
  // Back-compat: older configs may store px directly.
  // Heuristic: small values look like % of bg height; larger values look like px.
  return n <= 20 ? Math.round(bgHeight * (n / 100)) : Math.round(n);
}
...
export async function getFieldTempInfo(exifInfo: Record<string, any>, opt: GetFieldTempInfoOpt) {
  ...
  for (const filed of fileds) {
    tempFieldRecord[filed.key] = {
      ...
      font: filed.font && {
        ...filed.font,
        size: resolveFontSizePx(opt.bgHeight, filed.font.size),
      },
    };
  }
...
export function getTextTempList(temps: ITemp[], opts?: IGetTempsConfOpts): ITemp[] {
  return temps.map((temp) => ({
    ...
    font: {
      ...
      size: resolveFontSizePx(opts.bgHeight, temp.font.size),
    },
  })).filter((i) => i.use);
}
