...
const defaultOptions = (): IConfig['options'] => ({
  iot: false,
  landscape: false,
  solid_bg: false,
  solid_color: '#fff',
...
export async function getConfig() {
  ...
  if (defConf.code === 0) {
    config.update((v) => {
      const next = onConfigUpdate(v, defConf.data);
      // Always start with default parameters; keep non-parameter settings (output/fonts/templates) intact.
